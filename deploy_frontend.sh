...
read -p "Enter the name of the CloudFormation stack: " studentportal
...
api_gateway_endpoint=https://tv5xen36f2.execute-api.us-east-1.amazonaws.com/Prod
# Get the CloudFront Distribution ID from the stack
cloudfront_distribution_id=E24AXWDG134Z9E #$(aws cloudformation describe-stacks --stack-name "$stack_name" --query "Stacks[0].Outputs[?OutputKey=='CloudFrontDistributionId'].OutputValue" --output text)

# Get the S3 Bucket Name from the stack
s3_bucket_name=studentportal-websitebucket-zitpki7phh1f #$(aws cloudformation describe-stacks --stack-name "$stack_name" --query "Stacks[0].Outputs[?OutputKey=='WebS3BucketName'].OutputValue" --output text)
...
echo "REACT_APP_API_ENDPOINT=$api_gateway_endpoint" >> .env
